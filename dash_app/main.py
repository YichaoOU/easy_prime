...
df = pd.read_csv("plot_PCA_yli11_2019-08-19_projection.csv")
...
gene_df = pd.read_csv("yli11_2019-08-19_0c51dd9d63f4.csv")

gene_df_cols = gene_df.columns.tolist()
gene_df_cols[0] = 'Gene ID'
gene_df.columns = gene_df_cols
print (gene_df[gene_df.columns[:10]].head())
gene_labels = pd.read_csv("genes.tsv",sep="\t",header=None,index_col=0)
gene_df['Gene Name'] = gene_labels.loc[gene_df['Gene ID'].tolist()][1].tolist()
kmeans.fit(df[['x','y']])
df['color'] = ["kmean_%s"%(x) for x in list(kmeans.labels_)]


gene_name_df = gene_df.copy()
gene_df.index = gene_df['Gene ID']
gene_df = gene_df.drop(['Gene Name','Gene ID'],axis=1)
gene_name_df.index = gene_name_df['Gene Name']
gene_name_df = gene_name_df.drop_duplicates('Gene Name')
gene_name_df = gene_name_df.drop(['Gene Name','Gene ID'],axis=1)
...
app.layout = html.Div([
	...
	## selector
	html.Div([
			html.H4("Input a query gene below (Gene name or Ensembl gene ID). Scatter plot (UMAP) showing gene expression (size) distribution in different cell types (color)"),
			dcc.Input(
				id='gene_search',
				type='text',
				value='GATA1'
	...
	## graph
	html.Div([
		dcc.Graph(id="crossfilter-UMAP-scatter", style={"width": "100%", "display": "inline-block"})
	...
	html.Div([
		dcc.Graph(id="crossfilter-dist-plot", style={"width": "100%", "display": "inline-block"})
...
def update_output(ns,nb,g):
	try:
		selected_gene = gene_df.loc[g.upper()].T
	except:
		try:
			selected_gene = gene_name_df.loc[g.upper()].T
		except:
			return u'''%s not found'''%(g)
	return pd.DataFrame(selected_gene.describe()).to_string()
	

@app.callback(
	Output("crossfilter-UMAP-scatter", "figure"),
	[
	Input('gene_search', 'n_submit'),
	Input('gene_search', 'n_blur')
	],
...
def make_figure(ns,nb,g):
	...
	# selected_gene = selected_gene[selected_gene.columns[0]]
	# print (selected_gene.head())
	plot_df = df.copy()
	plot_df.index = plot_df['text']
	# print (selected_gene)
	plot_df['size'] = selected_gene
	plot_df['size'] = plot_df['size']/plot_df['size'].max()
	plot_df['size'] = plot_df['size'].apply(define_size)
	# print (plot_df.head())
	return px.scatter(plot_df,x="x",y='y',color='color',size='size',hover_data=['text'],color_discrete_sequence=color_set,opacity=0.5)
@app.callback(
	Output("crossfilter-dist-plot", "figure"),
	[
	Input('gene_search', 'n_submit'),
	Input('gene_search', 'n_blur')
	],
...
def make_figure2(ns,nb,g):
	...
	try:
		...
		selected_gene = gene_name_df.loc[g.upper()].T
	selected_gene = pd.DataFrame(selected_gene)
	# print (selected_gene.head())
	data = selected_gene[g.upper()].tolist()
	# data = [np.log2(x+1) for x in data]
	# print (selected_gene.head())
	# print (plot_df.head())
	return ff.create_distplot([data], [g],bin_size=1)
	# return px.histogram(selected_gene)
